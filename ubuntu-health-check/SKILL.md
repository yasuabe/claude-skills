...
1. **OS サポート状態** ★最優先
   - Ubuntu のバージョンが EOL（サポート終了）を迎えていないか
   - Ubuntu Pro / ESM の有効状態
   - EOL の場合、アップグレードパスを推奨に含める
...
4. **APT リポジトリ健全性**
   - リポジトリの GPG 鍵の期限切れ・欠落（`NO_PUBKEY` エラー）
   - `apt update` 時のエラー・警告
...
7. **ハードウェア健全性**
   - SMART によるディスク健康状態（smartmontools 未インストールの場合は導入を推奨）
   - CPU 温度（80°C超で警告）
   - NTP 時刻同期の状態
...
- `collect.sh` は `set +e` で動作し、個々のコマンドが失敗しても最後まで実行される。取得できた情報で分析を行う。
